...
function bindEvents() {
  ...
  window.addEventListener("pointermove", (event) => {
    ...
    canvas.style.cursor = state.dragging ? "grabbing" : state.hoverBody ? "pointer" : "grab";
  });
  ...
  window.addEventListener("pointerup", (event) => {
    ...
    if (state.dragging && !state.dragMoved && clickedBody) {
      cancelTourOnInteraction();
      focusBody(clickedBody);
    } else if (state.dragging && !state.dragMoved && !clickedBody) {
      state.selectedBody = null;
      updateInfoPanel(null);
    }
